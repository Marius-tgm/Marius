...
public class optOutput{
    ...
    public static boolean timeselect(double a, int Laenge){  		//figures out, wether the time of a data fits the scale for it to be drawn, a = Zeitpunkt,
    ...
    public static double round(double number, int _Hoehe){          // rounds the number to fit it into the graph (y-achse)
    
        if(number==0||number==10||number==100||number==1000||number==10000)    //they have to be treated differently, as they have one decimal place more, wenn die Werte genau 10 oderso sind, muessen die nicht mehr gerundet werden
			return (int)number;
        ...
        int[] rnumber = new int[4];     
        double hnumber;
        
        rnumber = distribute(number);			//rnumber[1] is always -1, since its describing the comma
    ...
    public static int[] distribute(double number){     //reads the first three characters of a number and put them into an array   
        ...
        for (int i=0; i<4; i++){ 
			...
			numberArray[i] = Character.getNumericValue(tmpString.charAt(i));  //es wird jede Stelle der zahl durchgegangen - der Zahlenwert der jeweiligen Nummer wird dann abgefragt und in das array gespeichert  (Ein Komma wird als -1 ausgewertet, da es kein int ist)  
        }   
        return numberArray;
    }
